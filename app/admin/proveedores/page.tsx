          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 560 }}>
            ...
            <tbody>
              {((suppliers ?? []) as unknown[]).length === 0 ? (
                <tr><td colSpan={5} style={{ padding: 24, textAlign: 'center', color: '#aaa', fontSize: 13 }}>No suppliers yet.</td></tr>
              ) : (
                ...
                (suppliers ?? []).map((s: any) => (
                  <tr key={s.id} style={{ borderTop: '1px solid #f0ede8' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <Link href={`/admin/proveedores/${s.id}`} style={{ fontSize: 12, color: '#334EAC', fontWeight: 600, textDecoration: 'none' }}>Ver →</Link>
                    </td>
                  </tr>
                ))
              )}
